//isdigit(字符)、 stoi(s)将字符串s转换为整数、list<IP>::iterator、 bool operator <、 
#include<iostream>
#include<string>
#include<list>
#include<cstring>
...
IP stringToIp(string&input){//将输入的IP地址转换为32位二进制IP地址，并求出前缀长度
    IP ip;
    string s="";
    int pow2[8]={1,2,4,8,16,32,64,128};//2的0~7次幂
...
void Merge1(list<IP>&ipAddress){//第一步合并，移除匹配集是前一IP地址子集的IP地址
    list<IP>::iterator i=ipAddress.begin(),j=ipAddress.begin();
	j++;    
    while(j!=ipAddress.end()){
        if(isChildCollection(*i,*j)){        	
            j=ipAddress.erase(j);//erase的作用是，使作为参数的迭代器失效，并返回指向该迭代器下一参数的迭代器
        }else{
            ++i;
            ++j;
        }
...
int main(){
    ...
    for(list<IP>::iterator i=ipAddress.begin();i!=ipAddress.end();i++){//输出IP地址
